...
function renderConferencePage(conference: Conference, posts: Post[]): string {
  ...
  const locationHtml = conference.location_address
    ? `<p><strong>Location:</strong> ${conference.location_address}</p>`
    : "";

  const dateHtml = `<p><strong>Dates:</strong> ${formatDate(conference.start_time)} – ${formatDate(conference.stop_time)}</p>`;
...
export async function handleComponentConferenceOptions(
  ...
  try {
    ...
    const optionsHtml = conferences
      .map((conf) => `<option value="${conf.id}">${conf.name}</option>`)
      .join("");
    const html =
...
export async function handleCreatePost(
  ...
  try {
    ...
    const cf_turnstile_response = formData.get(
    ...
    if (cf_turnstile_response) {
      ...
      turnstileBody.append(
        "remoteip",
        request.headers.get("CF-Connecting-IP") || "",
      );

      const turnVerify = await fetch(
    ...
    if (conferenceId === "new") {
      ...
      const newConfLocation = [newConfCity, newConfState].filter(Boolean).join(", ");
...
export async function handleMessageSend(
  ...
  try {
    ...
    return Response.redirect(
      `${new URL(request.url).origin}/post/${postId}`,
      303,
    );
  } catch (error) {
...
export async function handleMyPosts(
  ...
  try {
    // Query user's posts joined with conference info
    const query = `
      SELECT
        posts.id,
        posts.title,
    ...
    if (results.length === 0) {
      ...
      for (const post of results) {
        ...
        const confStart = formatDate(post.start_time);
        const confEnd = formatDate(post.stop_time);

        postsHtml += `
          <li class="my-post-item">
            <h3><a href="/post/${post.id}">${escapeHtml(post.title)}</a></h3>
            <p class="conference-info">
              <a href="/conference/${post.conference_slug}">${escapeHtml(post.conference_name)}</a>
              &middot; ${confStart} – ${confEnd}
            </p>
            <p class="created-info">Posted on ${date}</p>
          </li>
        `;
      }
      postsHtml += `</ul>`;
...
export async function handleSearch(
  ...
  const q = url.searchParams.get("conference")?.trim() || "";
  ...
  if (q) {
    conditions.push("conferences.name LIKE ?");
    bindings.push(`%${q}%`);
  }
  if (startParam) {
  ...
  let resultsHtml = "";
  let searched = q || startParam || endParam;
  ...
  try {
    ...
    if (searched && results.length === 0) {
      resultsHtml = `<p>No posts found matching your search.</p>`;
    } else {
      resultsHtml = results
        .map(
          (post) => `
        <div class="post-card">
          ...
          <small>
            ...
            · ${formatDate(post.start_time)} – ${formatDate(post.stop_time)}
          </small>
        </div>
      `,
        )
  ...
  const content = `
    <div class="site-page">
      ...
      <form method="GET" action="/search">
        <input type="text" name="conference" placeholder="Conference name" value="${escapeHtml(q)}" />
