...
The client script is loaded once, by `renderShell()` in `src/lib/shell.mjs`, which is where both the generated `base.njk` and `renderFullPage()` get it — so any page gets a working widget just by emitting `<div class="cf-turnstile" data-sitekey="...">`.
